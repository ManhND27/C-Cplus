                             ...
                             unsigned int numFunctions)
{
    ...
    if (numFunctions > sizeof(SALTS) / sizeof(*SALTS)) {
		return NULL;
	}
    ...
    filter->table = calloc((tableSize + 7) / 8, 1);
    ...
    filter->numFunctions = numFunctions;
    filter->tableSize = tableSize;
...
void bloomfilter_insert(BloomFilter *bloomfilter, BloomFilterValue value)
{
    ...
    unsigned int hash = bloomfilter->hashFunc(value);
    ...
    for (i=0; i<bloomfilter->numFunctions; ++i) {
        ...
        index = subhash % bloomfilter->tableSize;
...
void bloomfilter_read(BloomFilter *bloomfilter, unsigned char *array)
{
    ...
    unsigned int array_size = (bloomfilter->tableSize + 7) / 8;
...
void bloomfilter_load(BloomFilter *bloomfilter, unsigned char *array)
{
    ...
    unsigned int arraySize = (bloomfilter->tableSize + 7) / 8;

	/* Copy from the buffer of the calling routine. */

    memcpy(bloomfilter->table, array, arraySize);
}
...
BloomFilter *bloomfilter_union(BloomFilter *filter1, BloomFilter *filter2)
{
    ...
    if (filter1->tableSize != filter2->tableSize
            || filter1->numFunctions != filter2->numFunctions
            || filter1->hashFunc != filter2->hashFunc) {
		return NULL;
	}

	/* Create a new bloom filter for the result */

    BloomFilter *result = bloomfilter_new(filter1->tableSize,
                             filter1->hashFunc,
                             filter1->numFunctions);
    ...
    unsigned int arraySize = (filter1->tableSize + 7) / 8;
